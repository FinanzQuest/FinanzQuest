...
--   'user'         cash <-> asset, initiated by a user  (asset_id + price required)
--   'movement'     cash in/out of the depot, no asset   (asset_id + price NULL)
--                  deposit  = amount > 0  (e.g. reward, top-up, incoming transfer)
--                  withdraw = amount < 0  (e.g. payout, outgoing transfer)
--   'savings_plan' externally funded asset purchase     (asset_id + price required)
CREATE TYPE depots.TransactionType AS ENUM ('user', 'movement', 'savings_plan');
...
-- asset_id IS NULL marks a pure cash row ('movement').
CREATE TABLE IF NOT EXISTS depots.transactions (
  ...
  type       depots.TransactionType NOT NULL DEFAULT 'user',
  id         bigint GENERATED BY DEFAULT AS IDENTITY NOT NULL,
  ...
  CONSTRAINT Transactions_type_check CHECK (
       (type = 'user'         AND asset_id IS NOT NULL AND price IS NOT NULL)
    OR (type = 'movement'     AND asset_id IS NULL     AND price IS NULL)
    OR (type = 'savings_plan' AND asset_id IS NOT NULL AND price IS NOT NULL)
  )
);
...
-- Cash "expense" of a transaction, from the depot's point of view.
-- Positive = cash leaves the depot, negative = cash enters it.
-- This is the single place where money semantics per type are defined.
CREATE OR REPLACE FUNCTION depots.transaction_expense(
  t      depots.TransactionType,
  amount real,
  price  real
) RETURNS numeric LANGUAGE sql IMMUTABLE AS $$
  SELECT CASE t
    WHEN 'user'         THEN amount::numeric * price::numeric  -- buy: cash out, sell: cash in
    WHEN 'savings_plan' THEN 0::numeric                        -- externally funded (none -> asset)
    WHEN 'movement'     THEN -amount::numeric                  -- deposit +100 -> cash +100
  END;
$$;

CREATE OR REPLACE FUNCTION depots.transaction_affects_commission(t depots.TransactionType)
...
-- not on every query. asset_id IS NULL = the depot's cash "position".
-- -----------------------------------------------------------------------------
...
WITH daily_totals AS (
  SELECT
    depot_id,
    asset_id,                                    -- NULL = cash (movement rows)
    date(tstamp) AS tstamp,
    SUM(depots.transaction_expense(type, amount, price)) AS daily_expenses,
    SUM(amount::numeric)                                 AS daily_amount,
    SUM(
      CASE WHEN depots.transaction_affects_commission(type)
        THEN commission::numeric ELSE 0 END
    )                                                    AS daily_commission
...
-- NULLS NOT DISTINCT needs PG 15+; on older versions use COALESCE(asset_id, 0).
CREATE UNIQUE INDEX idx_agg_tx_mat_unique
  ON depots.aggregated_transactions_mat (depot_id, asset_id, tstamp) NULLS NOT DISTINCT;

-- -----------------------------------------------------------------------------
-- position_values_mat
-- Daily market value + profit per depot/asset. Reads from the matview above
-- so no window functions run at query time.
-- Cash rides along as a pseudo-position: asset_id IS NULL, no price,
-- market_value = 0. That keeps depots.values unchanged while letting
-- deposits/withdrawals flow into cash and total value.
-- -----------------------------------------------------------------------------
...
joined AS (
  SELECT
    ...
    t.running_expenses,
    ap.close AS price,
    CASE WHEN dr.asset_id IS NULL
      THEN 0::numeric                                   -- cash is not an asset holding
      ELSE t.running_amount * ap.close::numeric
    END AS market_value
  FROM date_range dr
  INNER JOIN LATERAL (
    ...
    WHERE t.depot_id  = dr.depot_id
      AND t.asset_id IS NOT DISTINCT FROM dr.asset_id   -- NULL-safe: keeps cash rows
  ...
  WHERE dr.asset_id IS NULL OR ap.close IS NOT NULL     -- cash has no price, keep it anyway
)
SELECT
  depot_id,
  tstamp,
  ...
  price,
  market_value,
  market_value - running_expenses - running_commission AS position_profit
FROM joined
ORDER BY depot_id, tstamp, asset_id
WITH DATA;

-- Required for CONCURRENTLY refresh
CREATE UNIQUE INDEX idx_position_values_mat_unique
  ON depots.position_values_mat (depot_id, asset_id, tstamp) NULLS NOT DISTINCT;
...
-- Cash rows (asset_id IS NULL) are excluded; they have no price.
CREATE OR REPLACE FUNCTION depots.get_position_profits(p_depot_id bigint)
RETURNS TABLE (
  depot_id       bigint,
  asset_id       bigint,
  symbol         text,
...
LANGUAGE sql STABLE AS $$
  WITH position_summary AS (
    SELECT
      t.depot_id,
      t.asset_id,
      SUM(t.amount::numeric) AS current_amount,
      SUM(CASE WHEN t.amount > 0
            THEN depots.transaction_expense(t.type, t.amount, t.price) + t.commission::numeric
    ...
    WHERE t.depot_id  = p_depot_id
      AND t.asset_id IS NOT NULL          -- skip cash movements
  ...
  latest_prices AS (
    SELECT DISTINCT ON (p.asset_id)
      p.asset_id,
      p.close::numeric AS current_price
    FROM api.asset_prices p
    WHERE p.close IS NOT NULL
      AND p.asset_id IN (SELECT ps.asset_id FROM position_summary ps)
    ORDER BY p.asset_id, p.tstamp DESC
  )
  SELECT
    ps.depot_id,
    ps.asset_id,
...
-- 'movement' rows come back with NULL asset columns (LEFT JOIN).
CREATE OR REPLACE FUNCTION depots.get_transactions_with_position(p_depot_id bigint)
RETURNS TABLE (
  depot_id       bigint,
  user_id        uuid,
  asset_id       bigint,
...
GRANT SELECT ON ALL TABLES IN SCHEMA depots TO authenticated;
